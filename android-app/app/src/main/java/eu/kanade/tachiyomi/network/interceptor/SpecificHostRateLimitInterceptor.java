...
public final class SpecificHostRateLimitInterceptor {
    ...
    public static OkHttpClient.Builder rateLimitHost(OkHttpClient.Builder builder, HttpUrl httpUrl, int permits, Duration period) {
        // Duration is Kotlin inline class - avoid calling its methods from Java to prevent symbol errors
        // Use 1 second default - rate limiting is non-critical for playback
        return builder.addInterceptor(new RateLimitInterceptor(httpUrl.host(), permits, 1, TimeUnit.SECONDS));
    }
    ...
    public static OkHttpClient.Builder rateLimitHost(OkHttpClient.Builder builder, String url, int permits, Duration period) {
        ...
        return builder.addInterceptor(new RateLimitInterceptor(host, permits, 1, TimeUnit.SECONDS));
    }
    ...
    public static RateLimitInterceptor rateLimitHost(HttpUrl httpUrl, int permits, Duration period) {
        return new RateLimitInterceptor(httpUrl.host(), permits, 1, TimeUnit.SECONDS);
    }
    ...
    public static RateLimitInterceptor rateLimitHost(String host, int permits, Duration period) {
        return new RateLimitInterceptor(host, permits, 1, TimeUnit.SECONDS);
    }
}
